...
async def test_manifest_resolves_same_name_and_renamed_process_environment_values(
    ...
    ).with_process_environment_access(
...
async def test_process_environment_access_is_runtime_only(
    ...
    with pytest.raises(ValueError, match="require an exact trusted binding"):
        untrusted.with_process_environment_access("TOKEN")
    manifest = untrusted.with_process_environment_access(("TOKEN", name))
    ...
    rebound = restored.with_process_environment_access(("TOKEN", name))
...
def test_process_environment_access_rejects_destinations_without_manifest_references() -> None:
    with pytest.raises(ValueError, match="no destination uses: 'UNUSED'"):
        Manifest().with_process_environment_access("UNUSED")
...
async def test_process_environment_access_grants_only_requested_destination_for_shared_source(
    ...
    trusted = manifest.with_process_environment_access(("TOKEN", source_name))
...
async def test_process_values_are_snapshotted_before_custom_resolvers(
    ...
    ).with_process_environment_access(("FIRST", first_name), ("SECOND", second_name))
