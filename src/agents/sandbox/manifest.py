    ...
    for key, entry in normalized.items():
        ...
        if (key, source_name) not in process_environment_access:
            raise _process_environment_error(
                ...
                "call Manifest.with_process_environment_access() from trusted runtime code"
            )
...
class Manifest(BaseModel):
    ...
    def with_process_environment_access(
        self,
        *bindings: str | tuple[str, str],
    ) -> "Manifest":
        """Grant runtime-only access to same-name or exact process environment bindings."""

        if not bindings:
            raise TypeError("At least one sandbox environment binding is required.")
        declared_bindings = self._declared_process_environment_bindings()
        bindings_by_destination = {
            destination: (destination, source_name)
            for destination, source_name in declared_bindings
        }
        unbound_destinations: set[str] = set()
        granted_bindings: set[tuple[str, str]] = set()
        for requested_binding in bindings:
            if isinstance(requested_binding, tuple):
                if len(requested_binding) != 2:
                    raise TypeError(
                        "Process environment binding tuples must contain destination and source."
                    )
                destination, source_name = requested_binding
            else:
                destination = requested_binding
                source_name = destination
            _validate_process_environment_name(destination)
            _validate_process_environment_name(source_name)
            binding = bindings_by_destination.get(destination)
            if binding is None:
                unbound_destinations.add(destination)
            elif binding != (destination, source_name):
                raise ValueError(
                    "Renamed process environment references require an exact trusted binding; "
                    f"use ({destination!r}, {binding[1]!r})"
                )
            else:
                granted_bindings.add(binding)
        if unbound_destinations:
            formatted = ", ".join(repr(name) for name in sorted(unbound_destinations))
            raise ValueError(
        ...
        trusted._process_environment_access = (
            self._process_environment_access | frozenset(granted_bindings)
        ) & declared_bindings
        return trusted
    ...
    def _copy_process_environment_access_from(self, *sources: "Manifest") -> None:
        allowed: set[tuple[str, str]] = set()
        for source in sources:
            allowed.update(
                source._process_environment_access & source._declared_process_environment_bindings()
            )
        self._process_environment_access = (
            frozenset(allowed) & self._declared_process_environment_bindings()
        )

    def _merge_process_environment_access(
        self,
        access: frozenset[tuple[str, str]],
    ) -> frozenset[tuple[str, str]]:
        merged = (
            self._process_environment_access | access
        ) & self._declared_process_environment_bindings()
        self._process_environment_access = merged
        return merged
